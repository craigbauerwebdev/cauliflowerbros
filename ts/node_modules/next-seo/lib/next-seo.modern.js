...
var buildVideo = ((video, context = false) => `{
      ...
      "thumbnailUrl": [
        ...
        ${video.hasPart ? `"hasPart": ${Array.isArray(video.hasPart) ? `[${video.hasPart.map(clip => `${buildClip(clip)}`)}]` : buildClip(video.hasPart)},` : ''}
        ${video.watchCount ? `${buildInteractionStatistic(video.watchCount)}` : ``}        
        ${video.publication ? `"publication": ${Array.isArray(video.publication) ? `[${video.publication.map(broadcastEvent => `${buildBroadcastEvent(broadcastEvent)}`)}]` : buildBroadcastEvent(video.publication)},` : ''}
...
export { ArticleJsonLd, BlogJsonLd, BreadCrumbJsonLd as BreadcrumbJsonLd, CarouselJsonLd, CorporateContactJsonLd, CourseJsonLd, DatasetJsonLd, defaultSEO as DefaultSeo, EventJsonLd, FAQPageJsonLd, JobPostingJsonLd, LocalBusinessJsonLd, LogoJsonLd, NewsArticleJsonLd, nextSEO as NextSeo, ProductJsonLd, RecipeJsonLd, SocialProfileJsonLd, VideoJsonLd };
